locals {
  ...
  resource_group_name_secondary      = element(coalescelist(data.azurerm_resource_group.rgrp_secondary.*.name, azurerm_resource_group.rg_secondary.*.name, [""]), 0)
...
data "azurerm_resource_group" "rgrp_secondary" {
  count = var.create_resource_group == false ? 1 : 0
...
resource "azurerm_resource_group" "rg_secondary" {
  ...
  name     = var.resource_group_name_secondary
  location = var.location
  tags     = merge({ "Name" = format("%s", var.resource_group_name_secondary) }, var.tags, )
}
...
#    id = azurerm_sql_server.secondary.0.id
    id = data.azurerm_sql_server.secondary.id
  }
